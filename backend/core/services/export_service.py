...
def export_poll_results_pdf(poll_id: int) -> BytesIO:
    ...
    try:
        ...
        raise ImportError("reportlab is required for PDF exports. Install with: pip install reportlab")
    ...
    results_table = Table(results_data, colWidths=[3 * inch, 1 * inch, 1.5 * inch, 0.8 * inch])
...
def export_vote_log(
    ...
    if format == "csv":
        ...
        # Column headers
        headers = ["Vote ID", "Timestamp", "Option", "User", "IP Address", "User Agent", "Valid"]
        if not anonymize:
        ...
        # Vote data
        for vote in votes:
            user_info = vote.user.username if vote.user and not anonymize else (
                anonymize_user_id(vote.user.id) if vote.user else "Anonymous"
            )
            ip_info = vote.ip_address if not anonymize else anonymize_ip(vote.ip_address)
        ...
        for vote in votes:
            ...
            if anonymize:
                vote_dict["user"] = anonymize_user_id(vote.user.id) if vote.user else "anonymous"
...
def export_analytics_report_pdf(poll_id: int) -> BytesIO:
    ...
    summary_data = [
        ...
        ["Participation Rate", f"{analytics.get('participation_rate', {}).get('rate', 0)}%"],
    ]
    
    if "average_time_to_vote" in analytics:
        summary_data.append(["Avg Time to Vote", f"{analytics['average_time_to_vote']:.2f} hours"])
    ...
    # Footer
    story.append(Spacer(1, 0.3 * inch))
    footer = Paragraph(f"Generated on {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal'])
    story.append(footer)
...
def export_audit_trail(
    ...
    if format == "csv":
        ...
        # Column headers
        writer.writerow([
            "ID", "Timestamp", "Method", "Path", "User", "IP Address", "Status Code", "Response Time (s)"
        ])
