...
from apps.analytics.models import AuditLog, FraudAlert, IPBlock, IPReputation, IPWhitelist
...
def get_recent_activity(limit: int = 50) -> List[Dict]:
    ...
    ).select_related('user', 'poll', 'option').order_by('-created_at')[:limit]
    ...
    recent_alerts = FraudAlert.objects.filter(
        created_at__gte=cutoff
    ).select_related('user', 'poll').order_by('-created_at')[:limit]
    ...
    # Recent IP blocks
    recent_blocks = IPBlock.objects.filter(
        blocked_at__gte=cutoff
    ).select_related('blocked_by').order_by('-blocked_at')[:limit]
...
def get_fraud_alerts_summary(limit: int = 20) -> Dict:
    ...
    risk_ranges = {
        ...
        "high": FraudAlert.objects.filter(risk_score__gte=60, risk_score__lt=80).count(),
        "medium": FraudAlert.objects.filter(risk_score__gte=40, risk_score__lt=60).count(),
        "low": FraudAlert.objects.filter(risk_score__lt=40).count(),
    }
    
    # Top polls with fraud alerts
    top_polls = FraudAlert.objects.values(
        'poll_id', 'poll__title'
    ).annotate(
...
def get_active_polls_and_voters(limit: int = 20) -> Dict:
    ...
    active_polls = Poll.objects.filter(
        ...
        starts_at__lte=now,
        ends_at__gte=now
    ).select_related('created_by').order_by('-cached_total_votes')[:limit]
    ...
    top_polls = Poll.objects.filter(
        is_active=True
    ).order_by('-cached_total_votes')[:10]
