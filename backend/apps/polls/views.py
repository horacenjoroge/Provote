...
class PollViewSet(RateLimitHeadersMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        include_drafts = (
            self.request.query_params.get("include_drafts", "false").lower() == "true"
        )
    ...
    def clone(self, request, pk=None):
        ...
        try:
            ...
            logger.info(
                f"Poll {poll.id} cloned to poll {cloned_poll.id} by user {request.user.id}"
            )
    ...
    def results_export(self, request, pk=None):
        ...
        """
        logger.info(
            f"results_export called: pk={pk}, format={request.query_params.get('format')}, path={request.path}"
        )
        try:
            poll = self.get_object()
        except Exception as e:
        ...
        large_export_threshold = getattr(
            settings, "LARGE_EXPORT_THRESHOLD", 1024 * 1024
        )  # 1MB default
        ...
        if export_format == "csv":
            ...
            try:
                ...
                response[
                    "Content-Disposition"
                ] = f'attachment; filename="poll_{poll.id}_results.pdf"'
                return response
    ...
    def results_live(self, request, pk=None):
        ...
        try:
            results = calculate_poll_results(
                poll.id, use_cache=False
            )  # Don't use cache for live updates
    ...
    def export_vote_log(self, request, pk=None):
        ...
        include_invalid = (
            request.query_params.get("include_invalid", "false").lower() == "true"
        )
        use_background = (
            request.query_params.get("background", "false").lower() == "true"
        )
        ...
        try:
            ...
            if export_format == "csv":
                ...
                response[
                    "Content-Disposition"
                ] = f'attachment; filename="poll_{poll.id}_vote_log.csv"'
                return response
            else:
    ...
    def export_analytics(self, request, pk=None):
        ...
        try:
            ...
            response[
                "Content-Disposition"
            ] = f'attachment; filename="poll_{poll.id}_analytics.pdf"'
            return response
    ...
    def export_audit_trail(self, request, pk=None):
        ...
        try:
            ...
            if export_format == "csv":
                ...
                response[
                    "Content-Disposition"
                ] = f'attachment; filename="poll_{poll.id}_audit_trail.csv"'
                return response
            else:
                return Response(json.loads(content), status=status.HTTP_200_OK)
    ...
    def analytics_timeseries(self, request, pk=None):
        ...
        cache_key = (
            f"poll_timeseries:{poll.id}:{interval}:{start_date_str}:{end_date_str}"
        )
