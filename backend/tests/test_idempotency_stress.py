...
@pytest.mark.stress
@pytest.mark.skipif(IS_SQLITE, reason="Idempotency stress tests require PostgreSQL. SQLite doesn't support concurrent writes.")
class TestIdempotencyStress:
    ...
    def test_1000_simultaneous_identical_votes(self, poll_with_choices, user):
        ...
        assert len(unique_vote_ids) == 1, f"Expected 1 unique vote, got {len(unique_vote_ids)}. Successful: {len(successful)}, Failed: {len(failed)}"
        
        # Verify only 1 new vote, rest are idempotent retries
        assert new_votes == 1, f"Expected 1 new vote, got {new_votes}"
        assert existing_votes == len(successful) - 1, f"Expected {len(successful) - 1} idempotent retries, got {existing_votes}"
        
        # Allow some failures due to SQLite limitations, but most should succeed
        if len(failed) > 0:
            success_rate = len(successful) / len(results)
            assert success_rate >= 0.9, f"Success rate too low: {success_rate:.2%}. Failures: {failed[:5]}"
    ...
    def test_network_retry_simulation(self, poll_with_choices, user):
        ...
        assert len(set(vote_ids)) == 1, f"Expected 1 unique vote, got {len(set(vote_ids))}"
        
        # Only first should be new
        assert is_new_flags[0] is True, "First vote should be new"
        assert all(not is_new for is_new in is_new_flags[1:]), "All subsequent votes should be idempotent"
    ...
    def test_concurrent_votes_same_voter_different_choices(self, poll_with_choices, user):
        """
    ...
    def test_race_condition_same_idempotency_key(self, poll_with_choices, user):
        ...
        assert len(unique_vote_ids) == 1, f"Expected 1 unique vote, got {len(unique_vote_ids)}"
        assert new_votes == 1, f"Expected 1 new vote, got {new_votes}"
        assert Vote.objects.filter(poll=poll, user=user).count() == 1
    ...
    def test_http_status_codes_under_retry_storm(self, poll_with_choices, user):
        ...
        assert status.HTTP_201_CREATED in status_values, f"No 201 status codes: {status_values}"
    ...
    def test_performance_under_retry_storm(self, poll_with_choices, user):
        ...
        if successful:
            ...
            print(f"  Min: {min_response_time*1000:.2f}ms, Max: {max_response_time*1000:.2f}ms")
